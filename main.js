...
this.GM_setValue = function (key, value) {
  // tr: burası bazı verilerin local Storageye kayıt edilmesi // en: this is where to save some data to local storage
    ...
    setTimeout(function () {
      javascript: (function loops() {
        ...
        if (!document.querySelector("#afk")) {
          ...
          $("#modmenupopUp .content .contentPopup.info").append(
            '<h5 style="color:black;">' + langlist.translator + ": " + langlist.translator_by + "</h5>"
          );
          $("#modmenupopUp .content .contentPopup.info").append(
            '<label class="select lang"><select name="language"><optgroup label="Languages"><option value="23">Azərbaycanca</option><option value="2">English</option><option value="8">Türkçe</option><option value="19">العربية</option><option value="1">Português</option><option value="999">Custom</option></optgroup></select></label>'
          );
          $(".select.lang").on("change", 'select[name="language"]', function () {
            lang = document.querySelector(".select.lang select").value;
            // loadwoldlist();
          });
          $("#modmenupopUp .content .contentPopup.info").append(
            '<label class="text" style="width: 100%; height: 40px;"><input class="woldurl" type="text" placeholder="URL (.json)" value=""></label>'
          );
          $(".contentPopup.info .text").on("change", ".woldurl", function () {
            customurl = document.querySelector(".woldurl").value;
            // loadwoldlist();
          });
          $("#modmenupopUp .content .contentPopup.info").append(
            '<div class="fieldset visibleRoom" style="width: 100%; height: 40px;"><img src="https://i.imgur.com/iBPf50O.png" style="position:relative;top:7px;right:5px;height:25px;"><span class="legend">' +
              langlist.Anti_AFK +
              ': </span><div class="switchFieldCheck" style="float:right;"><input type="checkbox" name="visible" id="afk"><label for="afk"></label></div></div>'
          );
          $(".switchFieldCheck").on("change", "#afk", function () {
            if (afkk == false) {
              afkk = true;
              $(".scrollElements:eq(1)").append('<div class="msg system">Anti AFK: <strong>Enabled</strong></div>');
            } else {
              afkk = false;
              $(".scrollElements:eq(1)").append('<div class="msg system">Anti AFK: <strong>Disabled</strong></div>');
            }
            antiafk();
          });
          $("#modmenupopUp .content .contentPopup.info").append(
            '<div class="fieldset visibleRoom" style="width: 100%; height: 40px;"><img src="https://i.imgur.com/HfLAgAS.png" style="position:relative;top:7px;right:5px;height:25px;"><span class="legend">' +
              langlist.Auto_kick +
              ': </span><div class="switchFieldCheck" style="float:right;"><input type="checkbox" name="visible" id="otokick"><label for="otokick"></label></div></div>'
          );
          $(".switchFieldCheck").on("change", "#otokick", function () {
            if (otokickk == false) {
              otokickk = true;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto kick: <strong>Enabled</strong></div>');
            } else {
              otokickk = false;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto kick: <strong>Disabled</strong></div>');
            }
            kicks();
          });
          $("#modmenupopUp .content .contentPopup.info").append(
            '<div class="fieldset visibleRoom" style="width: 100%; height: 40px;"><img src="https://i.imgur.com/it44rZS.png" style="position:relative;top:7px;right:5px;height:25px;"><span class="legend">' +
              langlist.Auto_Skip +
              ': </span><div class="switchFieldCheck" style="float:right;"><input type="checkbox" name="visible" id="otoskip"><label for="otoskip"></label></div></div>'
          );
          $(".switchFieldCheck").on("change", "#otoskip", function () {
            if (skipp == false) {
              skipp = true;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto skip: <strong>Enabled</strong></div>');
            } else {
              skipp = false;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto skip: <strong>Disabled</strong></div>');
            }
            skip();
          });
          $("#modmenupopUp .content .contentPopup.info").append(
            '<div class="fieldset visibleRoom" style="width: 100%; height: 40px;"><img src="https://i.imgur.com/VkvLxc0.png" style="position:relative;top:7px;right:5px;height:25px;"><span class="legend">' +
              langlist.Auto_Report +
              ': </span><div class="switchFieldCheck" style="float:right;"><input type="checkbox" name="visible" id="otoreport"><label for="otoreport"></label></div></div>'
          );
          $(".switchFieldCheck").on("change", "#otoreport", function () {
            if (reportt == false) {
              reportt = true;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto report: <strong>Enabled</strong></div>');
            } else {
              reportt = false;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto report: <strong>Disabled</strong></div>');
            }
            report();
          });
          $("#modmenupopUp .content .contentPopup.info").append(
            '<div class="Rainbow Drawing" style="width: 100%; height: 40px;"><img src="https://i.imgur.com/MH34Z4h.png" style="position:relative;top:7px;right:5px;height:25px;"><span class="legend">' +
              langlist.Rainbow_Drawing +
              ': </span><div class="switchFieldCheck" style="float:right;"><input type="checkbox" name="visible" id="renklicizimm"><label for="renklicizimm"></label></div><input name="renklicizimmms" id="renklicizimmms" type="number" style="position:relative;top:8px;float:right;right:5px; width: 20%;" max="100000" step="1" value="1000" class="renklicizimmms" oninput="amount.value=renklicizimmms.value;"></div>'
          );
          $(".switchFieldCheck").on("change", "#renklicizimm", function () {
            if (renklicizim == false) {
              renklicizim = true;
              $(".scrollElements:eq(1)").append(
                '<div class="msg system">Rainbow Drawing: <strong>Enabled</strong></div>'
              );
            } else {
              renklicizim = false;
              $(".scrollElements:eq(1)").append(
                '<div class="msg system">Rainbow Drawing: <strong>Disabled</strong></div>'
              );
            }
            rengareknkizim();
          });
          $(".Rainbow.Drawing").on("change", ".renklicizimmms", function () {
            renklicizimms = document.querySelector(".renklicizimmms").value;
          });

          $("#modmenupopUp .content .contentPopup.info").append(
            '<div class="Auto Answer" style="width: 100%; height: 40px;"><img src="https://i.imgur.com/ejCouCQ.png" style="position:relative;top:7px;right:5px;height:25px;"><span class="legend">' +
              langlist.Auto_Answer +
          ...
          $(".switchFieldCheck").on("change", "#autoanswerr", function () {
            if (autoanswer == false) {
              autoanswer = true;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto answer: <strong>Enabled</strong></div>');
            } else {
              autoanswer = false;
              $(".scrollElements:eq(1)").append('<div class="msg system">Auto answer: <strong>Disabled</strong></div>');
            }
            autoanswerloop();
          });
          $(".Auto.Answer").on("change", ".autoanswerms", function () {
            autoanswerms = document.querySelector(".autoanswerms").value;
          });
        ...
        if (!document.getElementById("author")) {
          $("#screens header div .logo").append(
            '<div style="border:1px solid black;border-radius:20px;padding:3px;width:200px;height:20px;font-size: 17px;text-align:center;background-color: 0, 106, 208;color:white;" id="author"></label>' +
              langlist.Cheat_İnjected +
              '<a style="font-weight:bold;color:black;font-size:14px;text-decoration:none;" class="author"></div>'
          );
        }
      })();
    }, 0);
            ...
            if (document.getElementById("canvas")) {
              ...
              if (isinput == false) {
                if (woldid < sendfound.length) {
                  ...
                  if (
                    document.querySelector("#answer .history .scroll .scrollElements .msg:last-child strong")
                      .textContent ==
                    document.querySelector(".user.you .infosPlayer .nick") + " " + sendfound[woldid]
                  )
                    return;
                  woldid = woldid + 1;
                }
...
function loop() {
  setTimeout(function () {
    javascript: (function loops() {
      ...
      if (document.getElementById("hint")) {
        ...
        if (document.querySelector("#hint button")) {
          if (document.getElementById("ara")) {
            ...
            $(".trbuttontr").html(
              '<div id="ara"><body><button type="button" onclick="parent.open(\'https://www.google.com/search?tbm=isch&tbs=itp:lineart&q=' +
                InputText +
                "');\">" +
                langlist.search +
                "</button></body></div>"
      ...
      if (lang == 999) {
        if (document.querySelector(".contentPopup.info .text")) {
          $(".contentPopup.info .text").css({ display: "block" });
        }
      } else {
        if (document.querySelector(".contentPopup.info .text")) {
          $(".contentPopup.info .text").css({ display: "none" });
        }
      }
      ...
      if (document.querySelector("#kemlimelistesi #screens")) {
        if (!document.querySelector("#__next .contribute .center")) {
          $("#__next").prepend(
            '<div class="contribute"><div><div class="center"><span><h3>Gartic.io hack!</h3><p>İf the page does not open, refresh 1 how many times</p></span><a href="https://github.com/anonimbiri/gartic.io-hack" target="_blank">Github source code</a></div></div><div>'
          );
        }
      }
      ...
      if (!document.getElementById("tamekran")) {
        ...
        $("#trbuttontr3").prepend(
          '<button id="tamekran" class="closing" ><img src="https://i.imgur.com/wm4kFLf.png" style="width:25px;height:25px;" /><span class="tooltip">' +
            langlist.Full_screen +
            "</span></button>"
        );
        $("#trbuttontr3").on("click", "#tamekran", function () {
          showfullscreen();
        });
      }
      if (!document.getElementById("modmenu")) {
        ...
        $("#trbuttontr4").prepend(
          '<button id="modmenu" class="closing" ><img src="https://i.imgur.com/4Dk6c7r.png" style="width:25px;height:25px;" /><span class="tooltip">Mod Menu (Ctrl + M)</span></button>'
        );
        $("#trbuttontr4").on("click", "#modmenu", function () {
          menu();
        });
      }
      ...
      if (!document.getElementById("odadegis")) {
        ...
        $("#5trbuttontr").append(
          '<button id="odadegis" class="closing" ><img src="https://i.imgur.com/HZsuXxP.png" style="width:25px;height:25px;" /><span class="tooltip">' +
            langlist.Change_room +
            "</span></button>"
        );
        $("#5trbuttontr").on("click", "#odadegis", function () {
          changeroom();
        });
      }
      // tr: mobil menü açmak için button // en: button to open mobile menu
      if (!document.querySelector(".Mobile.Mod.Menu")) {
        $(".actionsMobile").prepend(
          '<button class="Mobile Mod Menu"><img src="https://i.imgur.com/JodxI35.png" style="width:25px;height:25px;" /></button>'
        );
        $(".actionsMobile").on("click", ".Mobile.Mod.Menu", function () {
          menu();
        });
      }
      if (!document.querySelector("#popUp .loading #loadingimage")) {
        $("#popUp .loading").append(
          '<div id="loadingimage"></div><img src="https://i.imgur.com/oXqL49V.gif" style="position:relative;left:11px;top:-100px;width:80px;height:80px;">'
        );
      }
      ...
      if (!document.getElementById("sensinbu")) {
        ...
        $("#sensinbu").html(
          '<img src="https://i.imgur.com/tZ5QPu8.png" style="position:relative;left:-20px;width:30px;height:30px;">'
        );
      }
      ...
      if (!document.getElementById("trbuttontr2")) {
        ...
        $("#trbuttontr2").html(
          '<button class="btYellowBig ic-copylink" ><strong>' + langlist.copy_nickname + "</strong></button>"
        );
      }
      $("#trbuttontr2").on("click", ".btYellowBig.ic-copylink", function () {
        copynickname();
      });

      if (!document.getElementById("sürüklebırak")) {
        $("#events").append(
          '<div id="sürüklebırak" style="display: none; height: 100%; width:100%; background-repeat: no-repeat; background-size: auto; text-align: center;color: white;border:1px solid rgba(93,136,153,255);border-radius:10px;background-color:black; opacity: 50%"><h4 style="margin: auto;text-align: center;padding: 250px;font-size:30px">' +
            langlist.draw_the_image +
            " (beta)</h4></div>"
        );
      }
      ...
      if (document.querySelector("#answer .history .scroll .scrollElements .msg strong")) {
        ...
        if (yazankişi === seninnickin) {
          if (yazılanmesaj === blacklist2) {
            ...
            $(
              "#kemlimelistesi div button:contains('" + yazılanmesaj.replace(seninnickin, "").replace(" ", "") + "')"
            ).css({ backgroundColor: "green", color: "white" });
            $("#kemlimelistesi div button:contains('" + yazılanmesaj.replace(seninnickin, "").replace(" ", "") + "')")
              .detach()
              .appendTo("#kemlimelistesi #hintknownbuttons");
          }
        }
      }
      ...
      if (!document.querySelector("#kemlimelistesi #hintbuttons button")) {
        if (!document.querySelector("#kemlimelistesi #hintknownbuttons button")) {
          if (!document.querySelector("#kemlimelistesi #hintbuttons h5")) {
            $("#kemlimelistesi #hintbuttons").append(
              '<h5 style="color:red;">word not found try pressing the refresh button in the corner</h5>'
            );
          }
        }
      }
      if (!document.getElementById("kemlimelistesi")) {
        if (document.querySelector(".logo")) {
          ...
          if (document.getElementById("canvas")) {
            ...
            $(".nextCenter").append(
              '<div id="kemlimelistesi" style="z-index: 2; margin-left: auto;margin-right: auto; height: 600px; overflow: scroll; width: 200px;border:1px solid rgba(93,136,153,255);border-radius:10px;padding:20px;background-color: rgba(159,189,201,255);"></div>'
            );
            $("#kemlimelistesi").append(
              '<button class="pin worldlist button" style="margin-right: 0%; float:right; position: relative;" data-toggle="tooltip" title="Pin"><img src="https://i.imgur.com/Bu7ERLd.png"></button>'
            );
            $("#kemlimelistesi").append(
              '<button class="refresh world button" style="margin-right: 2%; float:right; position: relative;" data-toggle="tooltip" title="Refresh Word"><img src="https://i.imgur.com/vwz9J2k.png"></button>'
            );
            $("#kemlimelistesi").append('<div id="hintbuttons" ></div>');
          }
        }
        $("#kemlimelistesi").on("click", ".pin.worldlist", function () {
          (async () => {
            let pins = await GM_getValue("pinworldlist");
            if (pins == null) {
              GM_setValue("pinworldlist", "true");
              $(".pin.worldlist img").attr("src", "https://i.imgur.com/AR0yw1X.png");
            } else {
              GM_deleteValue("pinworldlist");
              $(".pin.worldlist img").attr("src", "https://i.imgur.com/Bu7ERLd.png");
            }
          })();
        });
        $("#kemlimelistesi").on("click", ".refresh.world", function () {
          $("#kemlimelistesi div").remove();
          processCurrentWord();
        });

        if (document.querySelector("#kemlimelistesi")) {
          processCurrentWord();
        }
      }
      $("#kemlimelistesi div").on("click", "button", function () {
        var fired_button = $(this).text();
        sendmesaj(fired_button);
      });
      ...
      if (document.getElementById("hint")) {
        if (!document.querySelector("#hint button")) {
          ...
          var InputText = InputText2.replace(/<span><\/span>/gi, " _ ")
            .replace(/<span>/gi, "")
            .replace(/<\/span>/gi, "")
            .replace(/<div class="word">/gi, "")
            .replace(/<\/div>/gi, " ㅤ "); ///<span></span>/g
          var InputText3 = InputText2.replace(/<span><\/span>/gi, "_")
            .replace(/<span>/gi, "")
            .replace(/<\/span>/gi, "")
            .replace(/<div class="word">/gi, "")
            .replace(/<\/div>/gi, ""); ///<span></span>/g
          ...
          if (len == hitlen) {
            inputChat.style.borderColor = "#2baa2b";
            inputChat.style.borderWidth = "3px";
            $("#yazım").css({ color: "#2baa2b" });
          } else {
            inputChat.style.borderColor = "#ff5d64";
            inputChat.style.borderWidth = "3px";
            $("#yazım").css({ color: "#ff5d64" });
          }

          if (öncekiyazı == InputText3) {
          } else {
          ...
          if (!document.getElementById("cavapyardımcısı")) {
            ...
            $("#cavapyardımcısı").append(
              '<a id="yazım" style="position:relative;left:10;top:5px;width:30px;height:30px;font-weight:bold;color:black;font-size:20px;text-decoration:none; target=" blank"="">' +
                InputText +
                charCount +
                "</a>"
            );
        ...
        if (inputChat == null) {
          ...
          inputChat.style.borderColor = null;
          inputChat.style.borderWidth = null;
        }
        (async () => {
          let pins = await GM_getValue("pinworldlist");
          if (pins == null) {
...
function menu() {
  ...
  $("#modmenupopUp").append(
    '<div class="content" style="position: absolute;"><button class="close"></button><div class="title"><h3>Mod Menu</h3></div><div class="contentPopup info"></div></div>'
  );
  $("#modmenupopUp .content").append(
    '<a onclick="parent.open(\'https://github.com/anonimbiri/gartic.io-hack\');" style="cursor: pointer;position:relative;top:70px;width:290px;background-color: rgba(64, 120, 192);" class="loginGoogle"><img src="https://i.imgur.com/HVXUQfZ.png" style="position:relative;left:15px;width:25px;height:25px;"><span>' +
      langlist.Github_source_code +
      "</span></a>"
  );
  setTimeout(function () {
    if (!document.querySelector("#modmenupopUp .content div #afk")) {
      $("#modmenupopUp .content").append(
        '<a style="font-weight:bold;color:red;font-size:20px;text-decoration:none; target=" blank"="">menu failed to load please refresh the page and wait a moment</a>'
      );
    }
  }, 1000);
}
...
function arakaplanyap() {
  if (backgroundimage === false) {
    ...
    if (src.match(/\.(jpeg|jpg|gif|png)$/) != null) {
      ...
      sendpopup("Error", "image not found", "https://i.imgur.com/egsP4I3.gif");
    }
  } else {
...
function sendpopup(title, text, img) {
  if (!document.getElementById("uyarıpopUp")) {
    ...
    $("#uyarıpopUp").append(
      `<div class="content" style="position: absolute;"><div class="title"><h3>${title}</h3></div><div class="contentPopup confirm"><div class="lottie"><img src="${img}" style="width:150px;height:150px;"></div><span class="legend">${text}</span></div><div class="buttons"><button class="btYellowBig ic-yes ic-okeys"><strong>${langlist.ok}</strong></button></div></div>`
    );
    $(".content").on("click", ".ic-okeys", function () {
      $("#popUp").css({ display: "none" });
      $("#uyarıpopUp").remove();
    });
    $("#modmenupopUp").remove();
...
function loadwoldlist() {
  // tr: burası github sayfamdan kelime listesini yüklediğimiz yer eski sürümlerde buradan yüklemiyorduk // en: this is where we loaded the wordlist from my github page we didn't load it from here in older versions
  ...
  if (lang == 999) {
    if (customurl) {
      $.getJSON(customurl, function (data) {
        Wordslist = data;
        console.log(
          `%c %cCustom wordlist loaded %c %c url: ${customurl} %c `,
          "background: #005900; padding:8px 2px;",
          "color: whitle; background: black; padding:5px 5px;",
          "background: #005900; padding:8px 2px;",
          "color:whitle; background: green; padding:5px 5px;",
          "background: #005900; padding:8px 2px;"
        );
        ...
        sendpopup(
          "Error",
          `Encountered a problem loading the word list\nerror description: ${textStatus}`,
          "https://i.imgur.com/JQGPXAb.gif"
        );
        console.log(
          `%c %cEncountered a problem loading the word list %c %c error description: ${textStatus} %c `,
          "background: #910000; padding:8px 2px;",
          "color: whitle; background: black; padding:5px 5px;",
          "background: #910000; padding:8px 2px;",
          "color:whitle; background: red; padding:5px 5px;",
          "background: #910000; padding:8px 2px;"
        );
      });
    }
    ...
    if (lang == 8) {
      ...
      listtext = "https://api.jsonbin.io/v3/b/64fd62758d92e126ae69ba0c";
    } else if (lang == 23) {
    ...
    $.getJSON(listtext, function (data) {
      console.log(
        `%c %cLoad worldlist %c %c url: ${listtext} %c `,
        "background: #005900; padding:8px 2px;",
        "color: whitle; background: black; padding:5px 5px;",
        "background: #005900; padding:8px 2px;",
        "color:whitle; background: green; padding:5px 5px;",
...
async function sendmesaj(word) {
  // tr: mesajı gönderdiğimiz yani cevapları gönderdiğimiz yer // en: where we send the message, that is, where we send the replies
  if (word) {
...
function processCurrentWord() {
  // tr: kelimeleri filtrelediğim kısım // en: the part where I filter the words
  woldid = 0;
  $("#kemlimelistesi div").remove();
  $("#alphabet").remove();
  ...
  div2.innerHTML =
    '<h5 style="margin-top: 10px; font-size: 100%; text-align: center; color: green; border-radius: 8px;" <h5> Tested words</h5>';
  ...
  if (document.querySelector(".word")) {
    ...
    var currWord = text
      .replace(/<span><\/span>/gi, "_")
      .replace(/<span>/gi, "")
      .replace(/<\/span>/gi, "")
      .replace(/<div class="word">/gi, "")
      .replace(/<\/div>/gi, ""); //.replace("undefined","");
    ...
    for (let i = 0; i < found.length; i++) {
      ...
      button.setAttribute(
        "style",
        "margin-top:10px;font-size:100%;text-align:center;background-color:Grey;border-radius:8px;  min-width: 15ch;min-height: 20px;box-shadow:03px 5px rgba(0, 0, 0, 0.18);display:flex;"
      );
    ...
    $(document).ready(function () {
      (async () => {
        ...
        if (pins != null) {
          ...
          $("#alphabet").attr(
            "style",
            "z-index: 2; background-color: rgb(159, 189, 201); font-weight: bold; text-align: center; font-size:20px; right:0px; width:30px;"
          );
          ...
          $("#kemlimelistesi div button").each(function () {
            ...
            if (current != null) {
              ...
              if (current.toUpperCase() != previous) {
                ...
                $("#alphabet").append(
                  "<a href='#first_letter_" + current.toUpperCase() + "'>" + current.toUpperCase() + "</a><br/>"
                );
              }
            }
          });
...
let generateLines = function (img) {
  ...
  let imageData = imageHelper.scaleImage(img, {
...
class Color {
  ...
  static distance(color1, color2) {
    ...
    return Math.sqrt(
      ((512 + rMean) / 256) * Math.pow(rDiff, 2) + 4 * Math.pow(gDiff, 2) + ((767 - rMean) / 256) * Math.pow(bDiff, 2)
    );
  }
}
...
function drawImage(startX, startY, stopX, stopY, color) {
  // tr: still drawing under construction // en: still drawing under construction
...
document.documentElement.addEventListener(
  ...
  function (e) {
    ...
    if (e.target.id == "events") {
      //e.target.className e.srcElement e.target.id e.target.tagName
  ...
  function (e) {
    if (e.target.tagName == "H4") {
      ...
      if (html) {
        ...
        console.log(
          `%c %cImage uploaded from site %c %c url: ${imgSrc} %c `,
          "background: #005900; padding:8px 2px;",
          "color: whitle; background: black; padding:5px 5px;",
          "background: #005900; padding:8px 2px;",
          "color:whitle; background: green; padding:5px 5px;",
        ...
        if (e.dataTransfer.files[0].type.startsWith("image")) {
          ...
          console.log(
            `%c %cImage uploaded from your computer %c %c url: ${imgSrc} %c `,
            "background: #005900; padding:8px 2px;",
            "color: whitle; background: black; padding:5px 5px;",
            "background: #005900; padding:8px 2px;",
            "color:whitle; background: green; padding:5px 5px;",
          ...
          console.log(
            `%c %cAn error occurred while loading the image %c %c error description: the file extension may be different %c `,
            "background: #910000; padding:8px 2px;",
            "color: whitle; background: black; padding:5px 5px;",
            "background: #910000; padding:8px 2px;",
            "color:whitle; background: red; padding:5px 5px;",
